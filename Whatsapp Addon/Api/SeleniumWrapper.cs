...
*
*/
using G1ANT.Addon.Whatsapp.Api.Enums;
...
namespace G1ANT.Addon.Whatsapp
{
    public class SeleniumWrapper
    {
        ...
        public void Click(WhatsappCommandArguments search, TimeSpan timeout, bool waitForNewWindow = false)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
        ...
        public void TypeText(string text, WhatsappCommandArguments search, TimeSpan timeout)
        {
            var elem = GetElementInFrame(search, timeout);
            elem.SendKeys(text);
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
        ...
        public void PressKey(string keyText, WhatsappCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            var elem = GetElementInFrame(search, timeout);
        ...
        public void SetAttributeValue(string attributeName, string attributeValue, AttributeOperationType setAttributeType, WhatsappCommandArguments search, TimeSpan timeout)
        {
        ...
        public string GetAttributeValue(string attributeName, WhatsappCommandArguments search)
        {
        ...
        public string GetAttributeValue(string attributeName, string elementXPath, WhatsappIFrameArguments search)
        {
            var element = GetElementInFrame(new WhatsappCommandArguments()
            {
        ...
        public string GetHtml(WhatsappIFrameArguments search)
        {
            return GetAttributeValue("outerHTML", "//html", search);
        }

        public string GetInnerHtml(WhatsappCommandArguments search)
        {
            return GetAttributeValue("innerHTML", search);
        }

        public string GetOuterHtml(WhatsappCommandArguments search)
        {
            return GetAttributeValue("outerHTML", search);
        }

        public string GetTextValue(WhatsappCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        public DataTable GetTableElement(WhatsappCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        private IWebElement GetElementInFrame(WhatsappCommandArguments search, TimeSpan timeout)
        {
            PreCheckCurrentWindowHandle();
            if (!string.IsNullOrEmpty(search.IFrameSearch?.Value))
        ...
        public void CallFunction(string functionName, object[] arguments, string type, WhatsappCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            PreCheckCurrentWindowHandle();
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
