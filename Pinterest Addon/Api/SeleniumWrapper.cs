...
*
*/
using G1ANT.Addon.Pinterest.Api.Enums;
...
namespace G1ANT.Addon.Pinterest
{
    public class SeleniumWrapper
    {
        ...
        public void Click(PinterestCommandArguments search, TimeSpan timeout, bool waitForNewWindow = false)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
        ...
        public void TypeText(string text, PinterestCommandArguments search, TimeSpan timeout)
        {
            var elem = GetElementInFrame(search, timeout);
            elem.SendKeys(text);
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
        ...
        public void PressKey(string keyText, PinterestCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            var elem = GetElementInFrame(search, timeout);
        ...
        public void SetAttributeValue(string attributeName, string attributeValue, AttributeOperationType setAttributeType, PinterestCommandArguments search, TimeSpan timeout)
        {
        ...
        public string GetAttributeValue(string attributeName, PinterestCommandArguments search)
        {
        ...
        public string GetAttributeValue(string attributeName, string elementXPath, PinterestIFrameArguments search)
        {
            var element = GetElementInFrame(new PinterestCommandArguments()
            {
        ...
        public string GetHtml(PinterestIFrameArguments search)
        {
            return GetAttributeValue("outerHTML", "//html", search);
        }

        public string GetInnerHtml(PinterestCommandArguments search)
        {
            return GetAttributeValue("innerHTML", search);
        }

        public string GetOuterHtml(PinterestCommandArguments search)
        {
            return GetAttributeValue("outerHTML", search);
        }

        public string GetTextValue(PinterestCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        public DataTable GetTableElement(PinterestCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        private IWebElement GetElementInFrame(PinterestCommandArguments search, TimeSpan timeout)
        {
            PreCheckCurrentWindowHandle();
            if (!string.IsNullOrEmpty(search.IFrameSearch?.Value))
        ...
        public void CallFunction(string functionName, object[] arguments, string type, PinterestCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            PreCheckCurrentWindowHandle();
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
