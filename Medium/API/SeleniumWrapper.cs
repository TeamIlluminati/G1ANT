...
*
*/
using G1ANT.Addon.Medium.Api.Enums;
...
namespace G1ANT.Addon.Medium 
{
    public class SeleniumWrapper
    {
        ...
        internal void PressKey(MediumLoginCommand.Arguments arguments, TimeSpan value)
        {
            throw new NotImplementedException();
        }
        ...
        public void TypeText(string text, MediumCommandArguments search, TimeSpan timeout)
        {
            var elem = GetElementInFrame(search, timeout);
            elem.SendKeys(text);
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
        ...
        public void PressKey(string keyText, MediumCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            var elem = GetElementInFrame(search, timeout);
        ...
        public void SetAttributeValue(string attributeName, string attributeValue, AttributeOperationType setAttributeType, MediumCommandArguments search, TimeSpan timeout)
        {
        ...
        public string GetAttributeValue(string attributeName, MediumCommandArguments search)
        {
        ...
        public string GetAttributeValue(string attributeName, string elementXPath, MediumIFrameArguments search)
        {
            var element = GetElementInFrame(new MediumCommandArguments()
            {
        ...
        public string GetHtml(MediumIFrameArguments search)
        {
            return GetAttributeValue("outerHTML", "//html", search);
        }

        public string GetInnerHtml(MediumCommandArguments search)
        {
            return GetAttributeValue("innerHTML", search);
        }

        public string GetOuterHtml(MediumCommandArguments search)
        {
            return GetAttributeValue("outerHTML", search);
        }

        public string GetTextValue(MediumCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        public DataTable GetTableElement(MediumCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        private IWebElement GetElementInFrame(MediumCommandArguments search, TimeSpan timeout)
        {
            PreCheckCurrentWindowHandle();
            if (!string.IsNullOrEmpty(search.IFrameSearch?.Value))
        ...
        public void CallFunction(string functionName, object[] arguments, string type, MediumCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            PreCheckCurrentWindowHandle();
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
