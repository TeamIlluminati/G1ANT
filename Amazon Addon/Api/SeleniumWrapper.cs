...
*
*/
using G1ANT.Addon.Amazon.Api.Enums;
...
namespace G1ANT.Addon.Amazon
{
    public class SeleniumWrapper
    {
        ...
        public void TypeText(string text, AmazonCommandArguments search, TimeSpan timeout)
        {
            var elem = GetElementInFrame(search, timeout);
            elem.SendKeys(text);
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
        ...
        public string GetAttributeValue(string attributeName, AmazonCommandArguments search)
        {
        ...
        public string GetAttributeValue(string attributeName, string elementXPath, AmazonIFrameArguments search)
        {
            var element = GetElementInFrame(new AmazonCommandArguments()
            {
        ...
        public string GetHtml(AmazonIFrameArguments search)
        {
            return GetAttributeValue("outerHTML", "//html", search);
        }

        public string GetInnerHtml(AmazonCommandArguments search)
        {
            return GetAttributeValue("innerHTML", search);
        }

        public string GetOuterHtml(AmazonCommandArguments search)
        {
            return GetAttributeValue("outerHTML", search);
        }

        public string GetTextValue(AmazonCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        public DataTable GetTableElement(AmazonCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        private IWebElement GetElementInFrame(AmazonCommandArguments search, TimeSpan timeout)
        {
            PreCheckCurrentWindowHandle();
            if (!string.IsNullOrEmpty(search.IFrameSearch?.Value))
