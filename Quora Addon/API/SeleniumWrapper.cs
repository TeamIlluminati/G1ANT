...
*
*/
using G1ANT.Addon.Quora.Api.Enums;
...
namespace G1ANT.Addon.Quora
{
    public class SeleniumWrapper
    {
        ...
        public void Click(QuoraCommandArguments search, TimeSpan timeout, bool waitForNewWindow = false)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
        ...
        public void TypeText(string text, QuoraCommandArguments search, TimeSpan timeout)
        {
            var elem = GetElementInFrame(search, timeout);
            elem.SendKeys(text);
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
        ...
        public void PressKey(string keyText, QuoraCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            var elem = GetElementInFrame(search, timeout);
        ...
        public void SetAttributeValue(string attributeName, string attributeValue, AttributeOperationType setAttributeType, QuoraCommandArguments search, TimeSpan timeout)
        {
        ...
        public string GetAttributeValue(string attributeName, QuoraCommandArguments search)
        {
        ...
        public string GetAttributeValue(string attributeName, string elementXPath, QuoraIFrameArguments search)
        {
            var element = GetElementInFrame(new QuoraCommandArguments()
            {
        ...
        public string GetHtml(QuoraIFrameArguments search)
        {
            return GetAttributeValue("outerHTML", "//html", search);
        }

        public string GetInnerHtml(QuoraCommandArguments search)
        {
            return GetAttributeValue("innerHTML", search);
        }

        public string GetOuterHtml(QuoraCommandArguments search)
        {
            return GetAttributeValue("outerHTML", search);
        }

        public string GetTextValue(QuoraCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        public DataTable GetTableElement(QuoraCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        private IWebElement GetElementInFrame(QuoraCommandArguments search, TimeSpan timeout)
        {
            PreCheckCurrentWindowHandle();
            if (!string.IsNullOrEmpty(search.IFrameSearch?.Value))
        ...
        public void CallFunction(string functionName, object[] arguments, string type, QuoraCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            PreCheckCurrentWindowHandle();
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
