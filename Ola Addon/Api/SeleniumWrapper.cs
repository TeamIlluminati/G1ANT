...
*
*/
using G1ANT.Addon.Ola.Api.Enums;
...
namespace G1ANT.Addon.Ola
{
    public class SeleniumWrapper
    {
        ...
        public void Click(OlaCommandArguments search, TimeSpan timeout, bool waitForNewWindow = false)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
        ...
        public void TypeText(string text, OlaCommandArguments search, TimeSpan timeout)
        {
            var elem = GetElementInFrame(search, timeout);
            elem.SendKeys(text);
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
        ...
        public void PressKey(string keyText, OlaCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            var elem = GetElementInFrame(search, timeout);
        ...
        public void SetAttributeValue(string attributeName, string attributeValue, AttributeOperationType setAttributeType, OlaCommandArguments search, TimeSpan timeout)
        {
        ...
        public string GetAttributeValue(string attributeName, OlaCommandArguments search)
        {
        ...
        public string GetAttributeValue(string attributeName, string elementXPath, OlaIFrameArguments search)
        {
            var element = GetElementInFrame(new OlaCommandArguments()
            {
        ...
        public string GetHtml(OlaIFrameArguments search)
        {
            return GetAttributeValue("outerHTML", "//html", search);
        }

        public string GetInnerHtml(OlaCommandArguments search)
        {
            return GetAttributeValue("innerHTML", search);
        }

        public string GetOuterHtml(OlaCommandArguments search)
        {
            return GetAttributeValue("outerHTML", search);
        }

        public string GetTextValue(OlaCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        public DataTable GetTableElement(OlaCommandArguments search, TimeSpan timeout)
        {
            var element = GetElementInFrame(search, timeout);
        ...
        private IWebElement GetElementInFrame(OlaCommandArguments search, TimeSpan timeout)
        {
            PreCheckCurrentWindowHandle();
            if (!string.IsNullOrEmpty(search.IFrameSearch?.Value))
        ...
        public void CallFunction(string functionName, object[] arguments, string type, OlaCommandArguments search, TimeSpan timeout)
        {
            NewPopupWindowHandler popupHandler = new NewPopupWindowHandler(webDriver);
            PreCheckCurrentWindowHandle();
            if (string.IsNullOrEmpty(search.IFrameSearch?.Value) == false)
